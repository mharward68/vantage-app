...
**Last run:** Phase 2B / **Session 2B.13 — Company duplication: the website seed and the autocomplete.** Compartment: the company create path. Review Findings 10a + 10c. Ran under Michael's authorised cross-compartment exception, granted at boot.
**Then, in the SAME conversation and deliberately NOT a second build session: a long design pass with Michael** that produced **three new sessions (2B.19, 2B.18, 2B.20)**, settled the **company field semantics**, rewrote the run sheet, and closed the `.gitignore` snapshot gap. **No application code was written after 2B.13's close** — everything since is documents plus one `.gitignore` line.
**State:** `node --check app.js` clean and `check_ids.py` at its standing baseline of two (`{'export-backup-btn', 'restore-backup-input'}`) · `CACHE_NAME` **v110 → v111, CONFIRMED LIVE** (`caches.keys()` `['vantageprm-cache-v111']`, `controlled: true`) · `app.js` 15,226 → **15,312** / `index.html` 3,442 → **3,447** / `style.css` **4,739 unchanged** · **console is 5 lines per boot, all green, ZERO RED** · **user data byte-identical at 27,959** · ✅ **GIT IS CLEAN AND PUSHED as of 2026-09-02** — Michael ran it by hand and reported "everything pushed clean"; the eight-session run of unknown git state is OVER · deployed n/a
**Estimate vs actual:** the BUILD was sized **M / ~6 min / High** and ran **M**, on plan for the code and over on verification — **Michael's time ~2 minutes.** The code is ~15 lines of substance; the cost was proving a **native datalist popup** on screen, which cannot be screenshotted and froze the renderer once.

⚠️ **THE DESIGN PASS AFTERWARDS WAS FAR LARGER THAN THE SESSION AND IS THE REAL LESSON FOR CALIBRATION. Michael's time: roughly 45 minutes, against ~2 for the build.** It began with him trying the shipped feature and saying *"the email does not enter the company domain."* Diagnosing that found an **order-dependent seed** the whole Done-when had missed, **a third company-creation path**, **no way to create a company at all**, and **two fields plus a word for one concept.** **Estimate the CONVERSATION a shipped feature starts, not just the diff.** 2B.10's calibration should say this plainly.
...
- **Carried, unchanged:** `#btn-see-all-contacts` does not exist. Both `forceShowAll*` true blanks the directory. `pros-sarah` carries no `conference*` keys. The four always-on conference boxes read as grey ghosts. CampaignHub identifies itself twice. Dashboard/DataHub emptiness is `renderDashboardView()`'s `slice(0, 5)`. MediaHub's tag rail off the right edge. `.checkbox-scroller` inline `max-height: 350px`. `.tags-filter-scroller` `max-height: 400px`. ✅ **`.gitignore` snapshot-glob gap FIXED 2026-09-02** — see below; remove this from any carried list. `state.columnLayouts.taskhub.widths` carries `firstName: 0` and `lastName: 0`.
  **`#companies-datalist` is no longer dead markup — it is filled. Remove that line from any list of fossils.**
- ✅ **GIT IS CLEAN AND PUSHED, 2026-09-02.** Michael ran `git status` / `git add -A` / `git commit` / `git push` himself and reported it clean. **There is still no shell on this machine** — the bridge writes files and reads directories and cannot run anything — so a future session still cannot check git and must ASK rather than assume.
- ✅ **THE `.gitignore` SNAPSHOT GAP IS CLOSED.** The old dated pattern had **no leading wildcard**, so it matched a whole filename — and real snapshots are named `vantage_snapshot_2026-09-02_121548.json`, which it did **not** match. **Every snapshot was unignored in a PUBLIC repo.** Now covered by `vantage_snapshot*` plus a leading `*` on the dated rule, with the reasoning written into the file.
...
**Code (2B.13):** `app.js` (15,226 → **15,312**), `index.html` (3,442 → **3,447**), `sw.js` (**v110 → v111**). **`style.css` UNTOUCHED — 4,739 both ends.**

**Documents (the design pass after it):** `ai/phases/phase-2b-review-response-plan.md` (**+3 sessions: 2B.19, 2B.18, 2B.20**, and the field-semantics block), `ai/phases/phase-2b-RUNSHEET.md` (**rewritten** — stale 2A banner removed, all 20 sessions listed, a locked-decisions table added), `ai/BUILD_NOTES.md`, `ai/AIContext.md`, `ai/archive/2026-09-02_1227_AIContext.md` (new), **`.gitignore`** (snapshot gap closed). `index.html`'s two-session untouched run **ends here**, by five lines.
