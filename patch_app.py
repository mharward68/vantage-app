...
old_open_prospect = """      document.getElementById("pros-location").value = p.location || "";
...
new_open_prospect = """      document.getElementById("pros-location").value = p.location || "";
...
content = content.replace(old_open_prospect, new_open_prospect)

# 3. Update saveProspect
old_save_prospect = """  const loc = document.getElementById("pros-location").value.trim();
  const tagsVal = document.getElementById("pros-tags").value.trim();"""

new_save_prospect = """  const loc = document.getElementById("pros-location").value.trim();"""
content = content.replace(old_save_prospect, new_save_prospect)

old_save_prospect_tags = """  const tags = tagsVal ? tagsVal.split(",").map(t => t.trim()).filter(Boolean) : [];"""
new_save_prospect_tags = """  const tags = [...pendingProspectTags];"""
content = content.replace(old_save_prospect_tags, new_save_prospect_tags)

# 4. Update launchCampaign
old_launch_campaign = """  const tagsVal = document.getElementById("campaign-tags").value.trim();
...
new_launch_campaign = """  const mediaId = document.getElementById("campaign-media-select").value;
...
content = content.replace(old_launch_campaign, new_launch_campaign)

# 5. Clear pendingCampaignTags at the end of launchCampaign
old_launch_campaign_end = """  document.querySelectorAll("#campaign-contacts-checkboxes input[type='checkbox']").forEach(cb => cb.checked = false);

  renderCampaignsView();
}"""
new_launch_campaign_end = """  document.querySelectorAll("#campaign-contacts-checkboxes input[type='checkbox']").forEach(cb => cb.checked = false);

  pendingCampaignTags = [];
  renderPendingTags('campaign-display-tags', pendingCampaignTags);
  renderCampaignsView();
}"""
content = content.replace(old_launch_campaign_end, new_launch_campaign_end)

# 6. Rewrite Choose Tags Controller
old_choose_tags_ctrl = """/* ==========================================================================
...
content = content.replace(old_choose_tags_ctrl, new_choose_tags_ctrl)

# 7. Add Event Listeners for new tag choosers
old_dash_tags = """  // Associated Tags click & choose triggers
...
new_dash_tags = """  // Associated Tags click & choose triggers
  // 1. Prospect Modal
...
content = content.replace(old_dash_tags, new_dash_tags)

# 8. Initialize Campaign Tags Display on view load
old_switch_view = """  if (viewId === "prospects") renderProspectsView();
  else if (viewId === "media") renderMediaView();
  else if (viewId === "campaigns") renderCampaignsView();
  else renderDashboardView();"""

new_switch_view = """  if (viewId === "prospects") renderProspectsView();
  else if (viewId === "media") renderMediaView();
